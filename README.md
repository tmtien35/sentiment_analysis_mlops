...
*   **Thu Thập Dữ Liệu Huấn Luyện On-Demand & Bảng Ghi Suy Luận Thời Gian Thực Cặp Đôi (Paired On-Demand Scoring & Live Active Learning Ingestion):** Khu vực kiểm thử suy luận on-demand được bố trí ở cuối trang dashboard theo bố cục cặp đôi trực quan:
    * **Cột trái (Kiểm thử & Quick Verify):** Cho phép nhập văn bản đánh giá xe điện bất kỳ, bấm dự đoán qua FastAPI, nhận diện kết quả tức thì (kèm % confidence), và cung cấp tiện ích **`🎯 Quick Verify & Add to Data Train`** cho phép thẩm định nhãn và lưu thẳng vào tập dữ liệu huấn luyện chỉ với 1 cú click chuột.
    * **Cột phải (Bảng kiểm toán đa dòng & Phê duyệt hàng loạt):** Chuyển đổi bảng lịch sử suy luận sang dạng bảng tương tác đa dòng (`st.data_editor`) tích hợp cột dropdown **`Verified Label`** (`positive`, `neutral`, `negative`) và cột trạng thái đào tạo (`✅ In Data Train` vs `⏳ Pending Audit`):
      ...
      * **Nút `💾 Save Verified Labels to Data Train`:** Lưu hàng loạt tất cả các dòng đã chọn nhãn vào bảng `store_reviews` (`category='on_demand'`, `is_processed=1`) và `predictions`, đồng thời cập nhật trạng thái bảng ghi để đưa trực tiếp vào vòng lặp Active Learning (`ml/train_model.py`) trong chu kỳ tái huấn luyện tiếp theo.
