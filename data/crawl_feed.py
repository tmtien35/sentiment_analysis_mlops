...
def crawl_daily_reviews(ds: str = None, n_reviews: int = 20):
    ...
    # 2. Check for idempotency: if this exact date already has crawled reviews, skip to avoid duplicate runs
    with engine.connect() as conn:
        ...
        if crawled_count > 0:
            print(f"ℹ️  Scraper: Date '{ds}' already has {crawled_count} crawled reviews in 'store_reviews'.")
            print(" -> Skipping crawl for this date to ensure idempotency.")
            return
    ...
    # 6. Format and insert into store_reviews
    clean_date_str = ds.replace("-", "")
    new_records = []
    
    for i, (_, row) in enumerate(sampled_df.iterrows(), 1):
        rev_id = f"FEED_{clean_date_str}_{i:03d}"
